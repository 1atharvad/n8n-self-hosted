...
class TextToVoice:
    ...
    def synthesize_stream(
        self, text: str, voice: str = "am_michael"
    ) -> Generator[bytes, None, None]:
        """
        Yield WAV bytes one sentence chunk at a time.
        Lets the client start playing audio before the full response is ready —
        this is what makes the voice agent feel real-time like ChatGPT voice.
        """
        for chunk in self.chunk_text(text):
            audio, sr = self.kokoro_tts.create(chunk, voice=voice)
            buf = io.BytesIO()
